...
def workspaces():
    ...
    m_i = bindsym("Mod1+{i}", "workspace {i}:{name}")
    m_shift_i = bindsym("Mod1+Shift+{i}",
                        "move container to workspace {i}:{name}")

    return lines("# Workspaces",
                 *[lines(m_i, m_shift_i).format(i=i, name=name) + "\n"
...
def write_conf():
    ...
    config = lines(workspaces(),
                   ...
                   "",
                   bindsym_exec("Control+mod1+l", "~/bin/lock"),
